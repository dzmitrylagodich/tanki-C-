...
enum Tmatter { ttStone, ttBrick }; // препятствия 
...
public:
    ...
    {
        RECT r = { px - 1 , py - 1 , px + 1 , py + 1 }; rct = r; use = 1;
    }
...
Tmap scr;
#define tankCnt 2
//Ttank tank[tankCnt] = { Ttank(1,11), Ttank(78,11) }; //обьект танка
...
{
    for (int i = 0; i < brickCnt; i++) // проходим по всем препятствиям
        if (brick[i].use &&
...
int main()
{
    ...
    do {
        ...
        tank[1].Move(38, 40, 37, 39, VK_RETURN);
